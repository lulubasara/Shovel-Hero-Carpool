...
export const initializeFirebase = async () => {
    appId = typeof window.__app_id !== 'undefined' ? window.__app_id : 'default-carpool-app';
    
    const configSource = window.__firebase_config;
    let firebaseConfig;

    if (configSource === undefined || configSource === null) {
        throw new Error("Firebase configuration is missing. Please ensure `window.__firebase_config` is defined, for example in a script tag in your index.html.");
    }

    if (typeof configSource === 'string') {
        if (configSource.trim() === '') {
            throw new Error("Firebase configuration string is empty. Please provide a valid JSON configuration.");
        }
        try {
            firebaseConfig = JSON.parse(configSource);
        } catch (e) {
            throw new Error("Firebase configuration string is not valid JSON. Please check the content of `window.__firebase_config`.");
        }
    } else if (typeof configSource === 'object') {
        firebaseConfig = configSource;
    } else {
        throw new Error("Firebase configuration has an unsupported type. It must be a JSON string or an object.");
    }
    
    if (!firebaseConfig || typeof firebaseConfig.apiKey !== 'string' || !firebaseConfig.apiKey) {
        throw new Error("Firebase configuration is invalid or missing the 'apiKey' property.");
    }
    
    if (firebaseConfig.apiKey === 'REPLACE_WITH_YOUR_FIREBASE_API_KEY') {
        throw new Error("偵測到預設的 API 金鑰。請在 index.html 檔案中將 'REPLACE_WITH_YOUR_FIREBASE_API_KEY' 替換為您 Firebase 專案的真實 Web API 金鑰。");
    }
    ...
    try {
        ...
        throw new Error("無法連接到認證服務。應用程式無法啟動，請檢查您的網路連線或 Firebase API 金鑰。");
    }
